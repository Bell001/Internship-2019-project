        ...
        then
	      ...
	      do
		      ...
		      then
			     ...
			     positions=$( echo $word | grep -o . | grep -n $temp_text | cut -d ':' -f 1 )
		      fi
		      ...
		      flag_T_in_word="F"
                      ...
                      then
                         ...
                         positions_1=$( echo "$true_key" | grep -o . | grep -n $temp_text | cut -d ':' -f 1 )
                         if [ -z "$positions_1" ]
                         then
                                      flag_T_in_word="F"
                         else
                      ...
                      then
                         ...
                         if [ "${flag_T_in_word}" = "F" ]
                         then
                                   true_key="${true_key}""${temp_text}"
                                   r_score=$(( 100/${#word}*${#positions[@]} ))
                                   c2=$(( ${#positions[@]}+c2 ))
				   echo "${r_score} ${temp_text}"
                         fi
                      fi
